...
const SEVERITY_ORDER = { critical: 0, warning: 1, notice: 2, info: 3 };
const VERSION = '1.4.0';
...
export function reportTerminal(results, options = {}) {
  ...
  for (const result of results) {
    ...
    // Default: CRITICAL + MITIGATED + WARNING only
    // --verbose: adds NOTICE and INFO
    const shown = showInfo
      ? findings
      : findings.filter(f => f.severity !== 'info' && f.severity !== 'notice');
    ...
    summaryRows.push({ name: plugin.name, critical: critCount, criticalMitigated: critMitigated, criticalActive: critActive, warning: warnCount, notice: noticeCount, info: infoCount });
  ...
  const header = hasMitigations
    ? `  ${'Plugin'.padEnd(nameWidth)}${'ACTIVE'.padStart(8)}${'MITIGATED'.padStart(11)}${'WARNING'.padStart(10)}`
    : `  ${'Plugin'.padEnd(nameWidth)}${'CRITICAL'.padStart(10)}${'WARNING'.padStart(10)}`;
  lines.push(color(c.dim, header));
  const divWidth = nameWidth + (hasMitigations ? 29 : 20);
  lines.push(color(c.dim, `  ${'-'.repeat(divWidth)}`));
  ...
  if (hasMitigations) {
    ...
    lines.push(`  ${'TOTAL'.padEnd(nameWidth)}${tActiveStr}${tMitStr}${tWarnStr}`);
  } else {
    ...
    lines.push(`  ${'TOTAL'.padEnd(nameWidth)}${totalCritStr}${totalWarnStr}`);
  }

  if (totalNotice > 0 || totalInfo > 0) {
    lines.push(color(c.dim, `  + ${totalNotice} notice, ${totalInfo} info (use --verbose to see)`));
  }
  lines.push('');
...
function severityBadge(severity) {
  switch (severity) {
    ...
    case 'notice':   return color(c.dim, '  NOTICE');
